...
** Implementation tasks
   ...
   + [ ] Load dds types and register through xtypes + converter
     + To be stored as scoped types (i.e. Kong::Foo)
   + [ ] Store data as xtypes DynamicData instead of DDS DynamicData
   + [ ] For a given type, iterate through its primitive types
   + [ ] Define real reference structure to map between dynamic types for dds/idl and fmu
   + [X] Load idl files
