...
struct CommandsInfo {
  ...
  int prepare_for_zip(const fs::path& alternative_output){
    ...
    if(!alternative_output.empty()){
      zip_output = alternative_output;
    }

    if (!zip_output.has_filename())
      zip_output /= default_zip;

    if (!zip_output.has_extension())
  ...
  int prepare_for_generate(){
    ...
    if (!fs::exists(model_description_tmpl)){
      std::cerr << "ModelDescription template file does not exist: " << model_description_tmpl << std::endl;
      return 1;
    }
...
int generate_xml(const CommandsInfo& info){
  ...
  // retrieve guid
  auto guid = generate_uuid(get_uuid_files(info.fmu_path, true), std::vector<std::string>{print_xml(doc)});
...
int main(int argc, const char* argv[]){
  ...
  args::ArgumentParser combo_parser(
      "dds-fmu repacker tool",
      "Run 'repacker COMMAND --help' for more information on a command.");
  ...
  args::Command archiver(
      commands, "zip",
      "Create an FMU zip archive of PATH.",
      [&](args::Subparser &parser)
      {
        args::ValueFlag<std::string> output_file(parser, "FILENAME", "Output file (Default: 'dds-fmu.fmu')", {'o', "output"});
  ...
  args::Command create(
      commands, "create",
      "Repackaging of 'dds-fmu' with customised configuration files. It runs 'generate' and then 'zip'.",
      [&](args::Subparser &parser)
      {
        args::ValueFlag<std::string> output_file(parser, "FILENAME", "Output file (Default: 'dds-fmu.fmu')", {'o', "output"});
        args::Flag force(parser, "force", "Overwrite if output file exists", {'f', "force"});
